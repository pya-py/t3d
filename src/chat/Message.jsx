...
const Message = ({ msg, previousDay }) => {
    ...
    useEffect(() => {
        ...
        setShowDate( (new Date(date)).getDate() !== previousDay || previousDay === 0);//     0 means this is the first message in the chat
    }, [date, previousDay]);

    return (
        <Fragment>
            {!showDate ? null : <Fragment><hr /><p className="messageDate">{persianDate}</p></Fragment>}
            <Row>
                <Col>
                    {msg.me && (
